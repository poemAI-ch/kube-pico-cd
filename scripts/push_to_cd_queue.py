...
def main():
    ...
    if kube_pico_cd_deploy_queue_name is None or kube_pico_cd_deploy_queue_name == "":
        raise Exception("KUBE_PICO_CD_DEPLOY_QUEUE_NAME environment variable is not set")
    ...
    _logger.info(f"Sent message for build {build_info['buildTimestamp']} to queue {kube_pico_cd_deploy_queue_name}")
